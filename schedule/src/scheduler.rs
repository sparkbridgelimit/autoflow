...
impl<E: Executor> Scheduler<E> {
    ...
    pub fn add_tasks(&mut self, tasks: Vec<Task>) {
        for task in tasks {
            let task_id = format!("{}-{}", task.trigger_id, task.run_at.timestamp());
...
mod tests {
    ...
    async fn test_empty_queue() {
        ...
        assert!(log_entries.is_empty(), "No tasks should be executed for an empty queue.");
    }
    ...
    async fn test_add_tasks_and_run() {
        ...
        let now = Utc::now();
        let tasks = vec![
            Task::new("task1", now + Duration::from_secs(1), "workflow1", "trigger1"),
            Task::new("task2", now + Duration::from_secs(2), "workflow2", "trigger2"),
            Task::new("task1", now + Duration::from_secs(1), "workflow1", "trigger1"), // Duplicate task for idempotency
        ];
    ...
    async fn test_future_task_not_executed_early() {
        ...
        let now = Utc::now();
        let tasks = vec![Task::new("task_future", now + Duration::from_secs(10), "workflow1", "trigger1")];
        ...
        // 检查日志，确保任务未提前执行
        let log_entries = log.lock().unwrap();
        assert!(log_entries.is_empty(), "Future tasks should not execute before their scheduled time.");
    }
    ...
    async fn test_high_frequency_scheduling() {
        ...
        for i in 0..10 {
            ...
            tasks.push(Task::new(&task_id, now + Duration::from_millis(i * 100), "workflow1", "trigger1"));
        }
    ...
    async fn test_duplicate_task_handling() {
        ...
        let tasks = vec![
            Task::new("task1", now + Duration::from_secs(1), "workflow1", "trigger1"),
            Task::new("task1", now + Duration::from_secs(1), "workflow1", "trigger1"), // Duplicate task
            Task::new("task2", now + Duration::from_secs(1), "workflow2", "trigger2"),
        ];
    ...
    async fn test_task_execution_order() {
        ...
        let now = Utc::now();
        let tasks = vec![
            Task::new("task1", now + Duration::from_secs(2), "workflow1", "trigger1"),
            Task::new("task2", now + Duration::from_secs(1), "workflow2", "trigger2"),
            Task::new("task3", now + Duration::from_secs(3), "workflow3", "trigger3"),
        ];
        ...
        assert_eq!(
            *log_entries,
            vec!["task2".to_string(), "task1".to_string(), "task3".to_string()]
        );
    }
}
